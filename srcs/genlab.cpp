...
int main(int argc, char * argv[])
{
	...
	{// Check for all Arguments that take no extra options
		...
		else if( std::string(argv[i]) == "--build")
		{
		...
		else if( std::string(argv[i]) == "--doc")
		{
			exitcode = handleDocArg();
		}
		else if( std::string(argv[i]) == "--pack")
		{
		...
		else 
		{
			...
			std::cout << "\n<Error>: '"<< agrv[1] << "' is not a recognized Argument.\n"
					<< "Try running:\n\t --help\nor\n\t--help <arg>\n\n";
			exitcode = -1;
		}
	}
