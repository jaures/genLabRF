...
int _strReplace(std::string& str, std::string ori, std::string rep)
{
	int p = str.find(ori);
	int cnt;
	
	for(cnt = 1; p != std::string::nopos; cnt++)
	{
		// Remove first instance of @param ori and insert
		//	into the location @param rep into @param str
		str = str.substr(0,p) + rep +
...
int _newlineCount(std::string str)
{
	...
	return _strReplace(str, "\n", "");
}


// Count Number of New Lines in a file
int _newlineCount(std::string file)
{
...
std::string _getMultiLineInput(int lineLimit = 32)
{
...
std::string _getMultiLineInput(std::ifstream& fr, int lineLimit = 32, std::string delim = "")
{
	std::string lines, line;
	lines = "";
