...
std::stringstream makeFileInit(GenFile gf)
{
	int err = NoError;
	std::stringstream ss;
	std::string line;
...
std::stringstream docFileInit(GenFile gf)
{
	std::stringstream ss;
...
// Creates a stream to write to a .TeX File 
//	@Returns a Stream to Content of the Doc File
std::stringstream importTests(TestFile tf)
{
	std::stringstream ss;
	...
	for(int i = 0; i < tf.tests.size(); i++)
	{
		...
		else
		{
			...
			for(int j = 1, int k = 1; k <= numOfLines; k += 15)
			{
				std::stringstream tmpSS;
